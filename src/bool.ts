...
function parseStrBool(str: string): boolean | undefined {
  if (boolTrueStringsUnsafe.includes(str)) return true;
  if (boolFalseStringsUnsafe.includes(str)) return false;
  return undefined;
}
...
export function boolParse(inp: any): boolean | undefined {
  ...
  if (typeof inp === 'string') return parseStrBool(inp);
  if (typeof inp === 'number') return parseNumberBool(inp);
  return undefined;
}
